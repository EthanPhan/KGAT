...
class Corpus:
    def __init__(self, args, train_data, validation_data, test_data, entity2id,
        ...
        adj_indices = np.array([train_data[1][0], train_data[1][1]], dtype=np.int32)
        adj_values = np.array(train_data[1][2], dtype=np.int32)
        ...
        if(get_2hop):
            ...
            cache_file = args.data + "2hop.pickle"
            if os.path.exists(cache_file):
                ...
                with open(cache_file, 'wb') as handle:
                    pickle.dump(self.node_neighbors_2hop, handle,
    ...
    def get_graph(self):
        graph = {}
        edges = np.transpose(self.train_adj_matrix[0])
        edge_values = np.expand_dims(self.train_adj_matrix[1], -1)

        all_tiples = np.concatenate([edges, edge_values], 1)

        for data in all_tiples:
            target, source, value = data
    ...
    def get_batch_nhop_neighbors_all(self, args, nbd_size=2):
        batch_sources = self.unique_entities_train
        node_neighbors = self.node_neighbors_2hop
